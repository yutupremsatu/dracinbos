...
async function run() {
    ...
    console.log('--- REELSHORT SEARCH ALPHABET ATTACK ---');

    // Alphabet a-z
    const chars = 'abcdefghijklmnopqrstuvwxyz'.split('');
    const queries = chars; // Keep it simple first

    for (const q of queries) {
        console.log(`Searching query: "${q}"...`);
        const html = await fetchURL(`https://www.reelshort.com/search/result?keywords=${q}`);

        const match = html.match(/<script id="__NEXT_DATA__" type="application\/json">(.*?)<\/script>/);
        if (!match) {
            console.log('  No Data Found (Blocked or Empty)');
            continue;
        }
        ...
        try {
            ...
            console.log(`  Found ${unique.length} titles.`);
